...
-- Uppercase first and second names

SELECT
	first_name,
	UPPER(first_name) AS first_name_upper,
	second_name,
	UPPER(second_name) AS second_name_upper
FROM employees;
...
SELECT
	...
	second_name,
	LOWER(second_name) AS second_name_upper
FROM employees;
...
SELECT 
	first_name || ' ' || second_name || ' makes ' || salary
FROM employees;
...
-- HAVING
-- After GROUP BY, return only the coffeeshops with more than 2 employees
SELECT
	shop_id, 
	COUNT(*) AS num_of_emp,
	ROUND(AVG(salary), 0) AS avg_sal,
...
FROM employees
GROUP BY shop_id
HAVING COUNT(*) > 2  -- filter, alter "where" after "group by"
ORDER BY num_of_emp DESC;

-- After GROUP BY, return only the coffeeshops with a minimum salary of less than 100000
SELECT
	shop_id, 
	COUNT(*) AS num_of_emp,
	ROUND(AVG(salary), 0) AS avg_sal,
...
INSERT INTO food_store VALUES (6, 'Happy Brew', NULL);
...
DELETE FROM food_store WHERE shop_id = 6;
...
-- Return all food store names, cities and countries
SELECT shop_name FROM food_store
UNION
SELECT city FROM locations
UNION
SELECT country FROM locations;
...
SELECT *
FROM employees
WHERE shop_id IN ( -- US coffeeshop_id's
	SELECT shop_id 
	FROM food_store
...
-- Return all employees who make over 35k and work in US food stores 
SELECT *
FROM employees
WHERE salary > 35000 AND shop_id IN ( -- US coffeeshop_id's
	SELECT shop_id
	FROM food_store
	WHERE city_id IN ( -- US city_id's
		SELECT city_id
